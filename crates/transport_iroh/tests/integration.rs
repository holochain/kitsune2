...
async fn network_stats() {
    ...
    ep_2.send_space_notify(ep_1_url, TEST_SPACE_ID, response.clone())
        .await
        .unwrap();
    tokio::time::timeout(
        Duration::from_secs(1),
    ...
    assert_eq!(
        ...
        1,
        "expected 1 peer urls but got {}",
        network_stats.transport_stats.peer_urls.len()
    );
    assert_eq!(network_stats.transport_stats.peer_urls[0], ep_2_url);
    assert_eq!(
        network_stats.transport_stats.connections.len(),
        1,
    ...
    assert_eq!(
        stats_ep_2.opened_at_s, opened_at_s,
        "unexpected opened at s {}",
        stats_ep_2.opened_at_s
    );
    assert!(
        stats_ep_2.is_webrtc,
        "expected direction connection but got false"
    );
}
