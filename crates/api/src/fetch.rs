...
pub fn serialize_ops(value: Vec<MetaOp>) -> bytes::Bytes {
    let ops = Ops::from(value).encode_to_vec();
    bytes::Bytes::copy_from_slice(&ops)
}

/// Deserialize list of ops.
pub fn deserialize_ops(value: bytes::Bytes) -> K2Result<Vec<MetaOp>> {
    let ops = Ops::decode(value).map_err(K2Error::other)?;
    Ok(ops.into())
}
...
pub trait Fetch: 'static + Send + Sync + std::fmt::Debug {
    ...
    /// Send held ops as a response to a requesting peer.
    fn respond_with_ops(
        &self,
        ops: Vec<OpId>,
        to: AgentId,
...
mod test {
    ...
    fn bytes_from_ops() {
        ...
        let bytes = serialize_ops(ops.clone());
        let ops_derialized = deserialize_ops(bytes).unwrap();

        assert_eq!(ops, ops_derialized);
    }
}
