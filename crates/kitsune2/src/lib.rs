...
mod test {
    ...
    use kitsune2_api::{
        ...
        BoxFut, DhtArc, K2Result, OpId, SpaceId, StoredOp, Timestamp,
    };
    use kitsune2_core::{
        factories::{
            ...
            MemOpStoreFactory, MemTransportFactory, MemoryOp,
        },
        Ed25519LocalAgent,
    };
    ...
    fn create_op_list(num_ops: u16) -> (Vec<StoredOp>, Vec<Bytes>, Vec<OpId>) {
        let mut stored_ops = Vec::new();
        let mut ops = Vec::new();
        let mut op_ids = Vec::new();
        for _ in 0..num_ops {
            let op = MemoryOp::new(Timestamp::from_micros(0), random_bytes(32));
            let op_id = op.compute_op_id();
            stored_ops.push(op.clone().into());
            ops.push(op.into());
            op_ids.push(op_id);
        }
        (stored_ops, ops, op_ids)
    }
    ...
    async fn two_node_gossip() {
        ...
        let (stored_ops_1, ops_1, op_ids_1) = create_op_list(100);
        space_1
            .op_store()
            .process_incoming_ops(ops_1.clone())
            .await
            .unwrap();
        // space_1.inform_ops_stored(stored_ops_1).await.unwrap();
        let (stored_ops_2, ops_2, op_ids_2) = create_op_list(100);
        space_2
            .op_store()
            .process_incoming_ops(ops_2.clone())
            .await
            .unwrap();
        // space_2.inform_ops_stored(stored_ops_2).await.unwrap();

        // Create 2 agents.
        let mut local_agent_1 = Ed25519LocalAgent::default();
        let mut local_agent_2 = Ed25519LocalAgent::default();
